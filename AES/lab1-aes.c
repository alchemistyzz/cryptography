...
//请补充代码
     ...
     for(int j=0; j<4; j++){
        array[0][j]=getNumFromSBox(array[0][j]);
        array[1][j]=getNumFromSBox(array[1][j]);
        array[2][j]=getNumFromSBox(array[2][j]);
        array[3][j]=getNumFromSBox(array[3][j]);
     }
...
//请补充代码
        for(int j=0; j<4; j++){
        array[0][j]=getNumFromS1Box(array[0][j]);
        array[1][j]=getNumFromS1Box(array[1][j]);
        array[2][j]=getNumFromS1Box(array[2][j]);
        array[3][j]=getNumFromS1Box(array[3][j]);
     }
...
//请补充代码
    //各列循环左移//这里没有问题
    leftLoop4int(array[0],0);
    leftLoop4int(array[1],1);
    leftLoop4int(array[2],2);
    leftLoop4int(array[3],3);
}
...
//请补充代码
    //各列循环右移//这里没有问题
    rightLoop4int(array[0],0);
    rightLoop4int(array[1],1);  
    rightLoop4int(array[2],2);
    rightLoop4int(array[3],3);
}
/**
 * 列混合
 */
...
//请补充代码
        ...
        for(int i = 0; i < 4; i++){
            for(int j = 0; j < 4; j++){
              ...
              temp[i][j] = GFMul(colM[i][0],array[0][j]) ^ GFMul(colM[i][1],array[1][j])
    ...
    int temp[4][4];
        for(int i = 0; i < 4; i++){
            for(int j = 0; j < 4; j++){
              ...
              temp[i][j] = GFMul(deColM[i][0],array[0][j]) ^ GFMul(deColM[i][1],array[1][j])
...
void addRoundKey(int array[4][4], int round)
{
    int in[4];
    for(int j=0;j<4;j++){
        in[j] = (array[0][j]<<24)+(array[1][j]<<16)+(array[2][j]<<8)+array[3][j];
    }
    for(int i=0;i<4;i++){
        in[i] = in[i] ^ w[4*round+i];
    } 
    int temp[4];
    for(int i=0;i<4;i++){
        splitIntToArray(in[i],temp);
        array[0][i] = temp[0];
        array[1][i] = temp[1];
        array[2][i] = temp[2];
        array[3][i] = temp[3];
...
//请补充代码
        int bit[4]; 
        splitIntToArray(num,bit);
        leftLoop4int(bit,1);
        bit[0]=getNumFromSBox(bit[0]);
        bit[1]=getNumFromSBox(bit[1]);
        bit[2]=getNumFromSBox(bit[2]);
        bit[3]=getNumFromSBox(bit[3]);
        num = (bit[0]<<24)+(bit[1]<< 16)+(bit[2]<< 8)+bit[3];
        num = num ^ Rcon[round] ;
        return num;
...
void extendKey(char *key)
{
   ...
   int temp[4];
   for(int i=0; i<4;i++){
        temp[0] = getIntFromChar(key[4*i+0]);
        temp[1] = getIntFromChar(key[4*i+1]);
        temp[2] = getIntFromChar(key[4*i+2]);
        temp[3] = getIntFromChar(key[4*i+3]);
        w[i] =  (temp[0]<< 24)+(temp[1]<< 16)+(temp[2]<< 8)+temp[3];

   }

   for(int i=4,round=0;i<44;i++){
        if(i%4==0){
            //循环左移
